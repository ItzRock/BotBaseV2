...
module.exports = (client) => {
    ...
    try {
        ...
        process.on("unhandledRejection", async (err) => {
            if(client.readyAt == null) return client.error(err)
            ...
            async function logRejectionToConsole(err, cleanedErr) {
                ...
                //client.channels.cache.get(client.config.debug.error).send(`\`\`\`js\n${cleanedErr.substring(0, 1500)}\n\`\`\``)
                const errorMsg = err.stack.replace(new RegExp(`${__dirname}/`, "g"), "./");
                client.error(`Unhandled Rejection: ${errorMsg}`);
            }
            ...
            for (let [rejectionName, rejectionTable] of Object.entries(regexMatches)) {
                ...
                channelId = (Array.isArray(channelId) ? channelId[0] : client.config.debug.error)
                ...
                if (!rejectionChannel || rejectionChannel === undefined || rejectionChannel === null) {
                    client.error(`Failed to fetch channel by id: ${rejectionChannel || "Unable to fetch id"}`);
                    continue;
                };
            ...
            for (let [rejectionName, rejectionData] of Object.entries(matchedRejections)) {
                ...
                try {
                    ...
                    //client.channels.cache.get(client.config.debug.error).send(`Another exception caught while trying to process the previous rejection: \`\`\`js\n${newErrorCleaned.substring(0, 1500)}\n\`\`\``)
                    client.error(`Another exception caught while trying to process the previous rejection: ${newError}`);
                }
            }
        });
    } catch (error) {
